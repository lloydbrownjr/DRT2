...
__global__ void init_camera_origins_for_frames(vec3 *camera_origins_for_frames, int num_frames_to_render) {
    const vec3& current_origin = vec3(13, 2, 3);
...
void benchmark_frame(int argc, char **argv, int image_height, int image_width, int samples_per_pixel, int num_frames_to_render) {
    ...
    if (num_procs < 2) {
        std::cerr << "This program requires at least 2 processes since 1 serves as the dispatcher.\n";
    }
    ...
    if (rank > 0) {
        std::cout << "Rank " << rank << ": Initializing GPU state " << std::endl;
        checkCudaErrors(cudaMalloc((void **)&d_rand_state, num_pixels * sizeof(curandState)));
        checkCudaErrors(cudaMalloc((void **)&d_rand_state2, 1*sizeof(curandState)));
        rand_init<<<1, 1>>>(d_rand_state2);
        checkCudaErrors(cudaGetLastError());
    ...
    if (rank == 0) {
        ...
        init_camera_origins_for_frames<<<1, 1>>>(camera_origins_for_frames, num_frames_to_render);
    }
    ...
    dim3 threads(tx, ty);
    if (rank > 0) {
        std::cout << "Rank " << rank << ": Initializing tiled rendering " << std::endl;
        render_init_tiled<<<blocks, threads>>>(image_width, image_height, d_rand_state);
    }
    checkCudaErrors(cudaGetLastError());

    // Rank 0 does no work and assigns.
    if (rank == 0) {
        ...
        while (true) {
            ...
            if (remaining_frames.size() == 0 || free_gpus.size() == 0) {
                ...
                if (gpu_for_that == rank) {
                    ...
                    std::cout << "Waiting for " << gpu_for_that << std::endl;
                    MPI_Status status;
        ...
        while (true) {
            ...
            std::cout << "Rank " << rank << ": Rendering frame" << std::endl;
            // get camera origin info
            update_camera_origin<<<1, 1>>>(camera_origin, d_camera);
            std::cout << "New camera origin: " << camera_origin << std::endl;
            // update d_camera
            ...
            std::cout << "Finished rendering frame" << std::endl;
            // render
            // send back frame
